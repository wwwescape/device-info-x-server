...
class ConnectionManager:
    ...
    def __init__(self) -> None:
        ...
        # Same "Here" shape again, for the Call Room (Messages header's call icon) — see
        # app/services/call_room_service.py for the room state this backs.
        self._in_call_room_screen: set[uuid.UUID] = set()
    ...
    async def disconnect(self, user_id: uuid.UUID, websocket: WebSocket) -> None:
        async with self._lock:
            ...
            if not connections:
                ...
                self._in_call_room_screen.discard(user_id)
    ...
    def set_in_call_room_screen(self, user_id: uuid.UUID, in_screen: bool) -> None:
        if in_screen:
            self._in_call_room_screen.add(user_id)
        else:
            self._in_call_room_screen.discard(user_id)

    def is_in_call_room_screen(self, user_id: uuid.UUID) -> bool:
        return user_id in self._in_call_room_screen
