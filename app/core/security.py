...
def sign_gif_proxy_url(url: str, expires_at: int) -> str:
    """HMAC over a real Klipy CDN URL + expiry, so `GET /gifs/proxy` only ever re-fetches a URL
    this server itself issued moments earlier (from its own search/trending response), not an
    open proxy for arbitrary URLs. Reuses `jwt_secret_key` as the HMAC key (same secret already
    trusted for access tokens) with a distinguishing namespace baked into the signed message, so a
    GIF-proxy signature can never be replayed as, or confused with, a JWT or any other HMAC use of
    the same secret — same "HMAC of a message string over a shared secret" shape as
    generate_turn_credentials above, just SHA-256 and a different secret."""
    message = f"gif_proxy:{expires_at}:{url}".encode()
    return hmac.new(settings.jwt_secret_key.encode(), message, hashlib.sha256).hexdigest()


def verify_gif_proxy_signature(url: str, expires_at: int, signature: str) -> bool:
    if time.time() > expires_at:
        return False
    return hmac.compare_digest(sign_gif_proxy_url(url, expires_at), signature)
