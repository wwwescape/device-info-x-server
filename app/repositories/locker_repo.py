...
async def get_by_original_message(
    db: AsyncSession, owner_id: uuid.UUID, original_message_id: uuid.UUID
) -> LockerItem | None:
    """Backs `locker_service.create_item`'s conflict-recovery path after `uq_locker_items_owner_message`
    rejects a duplicate insert — the pre-existing row this device's own dedupe check should have
    found, had it not raced a concurrent save."""
    result = await db.execute(
        select(LockerItem).where(
            LockerItem.owner_id == owner_id,
            LockerItem.original_message_id == original_message_id,
            LockerItem.deleted_at.is_(None),
        )
    )
    return result.scalars().first()


async def get_by_original_event_attachment(
    db: AsyncSession, owner_id: uuid.UUID, original_event_id: uuid.UUID, original_attachment_media_id: uuid.UUID
) -> LockerItem | None:
    """Same as [get_by_original_message] but for `uq_locker_items_owner_event_media`."""
    result = await db.execute(
        select(LockerItem).where(
            LockerItem.owner_id == owner_id,
            LockerItem.original_event_id == original_event_id,
            LockerItem.original_attachment_media_id == original_attachment_media_id,
            LockerItem.deleted_at.is_(None),
        )
    )
    return result.scalars().first()
