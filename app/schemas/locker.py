...
class LockerItemCreate(BaseModel):
    ...
    # Set only when saving from an existing message/event attachment — see LockerItem's own
    # doc comment for why these back the server-side dedupe constraint.
    original_message_id: uuid.UUID | None = None
    original_event_id: uuid.UUID | None = None
    original_attachment_media_id: uuid.UUID | None = None
