...
class MessageCreate(BaseModel):
    ...
    # LOCATION-only — a one-time GPS snapshot captured at send time.
    location_lat: float | None = None
    location_lng: float | None = None
    # Client-generated, shared across every sibling IMAGE/VIDEO message in one multi-select send —
    # groups them into one "album" client-side. Absent/null on every other message type.
    media_group_id: uuid.UUID | None = None
