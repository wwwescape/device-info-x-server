...
from app.models.calendar_event import CANCELLABLE_TYPES, CANCELLED_BY_BOTH, RECURRING_ALLOWED_TYPES, CalendarEvent
...
from app.services import auth_service, insight_markup_validator, release_service
...
async def add_insight(*, insight_text: str, username: str | None) -> None:
    """Creates a new DIX AI insight — manually-authored one-liner copy; DIX AI isn't real AI yet,
    so correctness is on whoever runs this command. Global by default (visible to both accounts);
    pass --user to scope it to that one account only — there's no pair-scoping concept since a
    server only ever has the 2 accounts, so "targeted" and "single-account" are the same thing."""
    markup_error = insight_markup_validator.validate_insight_markup(insight_text)
    if markup_error is not None:
        print(f"Invalid Markdown syntax: {markup_error}")
        print("Check your syntax: https://markdownlivepreview.dev/tools/markdown-validator")
        return
    async with async_session_factory() as db:
        target_id = None
        ...
        insight = await insight_repo.create(db, text=insight_text, target_user_id=target_id)
    scope = f"targeted at {username!r} only" if username else "global (both accounts)"
    print(f"Added insight {insight.id} — {scope}.")


async def list_insights(*, include_disabled: bool) -> None:
    """Read-only: lists every DIX AI insight, oldest first, with its id (needed for
    delete-insight/enable-insight/disable-insight/reset-insight-seen), enabled state, target, and
    how many accounts have already dismissed it."""
    async with async_session_factory() as db:
        insights = await insight_repo.list_all(db, include_disabled=include_disabled)
        if not insights:
            print("No insights yet.")
            return
        for insight in insights:
            target = "global"
            if insight.target_user_id is not None:
                target_user = await user_repo.get_by_id(db, insight.target_user_id)
                target = f"user:{target_user.username if target_user else insight.target_user_id}"
            seen_count = await insight_seen_repo.count_seen(db, insight.id)
            state = "enabled" if insight.enabled else "disabled"
            text_preview = insight.text if len(insight.text) <= 80 else insight.text[:77] + "..."
            print(f"{insight.id}  [{state}]  target={target}  seen_by={seen_count}  {text_preview!r}")


async def delete_insight(*, insight_id: uuid.UUID) -> None:
    """Permanently deletes a DIX AI insight — cascades its `InsightSeen` rows for every account
    (unlike disable-insight, which just stops future distribution)."""
    async with async_session_factory() as db:
        insight = await insight_repo.get_by_id(db, insight_id)
        if insight is None:
            print(f"No insight with id {insight_id}.")
            return
        await insight_repo.delete(db, insight)
    print(f"Deleted insight {insight_id}.")


async def set_insight_enabled(*, insight_id: uuid.UUID, enabled: bool) -> None:
    """Backs both enable-insight and disable-insight — the content-level on/off switch
    (`Insight.enabled`), deliberately a different axis from per-user seen-state. Doesn't touch any
    account's existing `InsightSeen` rows; use reset-insight-seen for "let them see it again"."""
    async with async_session_factory() as db:
        insight = await insight_repo.get_by_id(db, insight_id)
        if insight is None:
            print(f"No insight with id {insight_id}.")
            return
        await insight_repo.set_enabled(db, insight, enabled=enabled)
    print(f"{'Enabled' if enabled else 'Disabled'} insight {insight_id}.")


async def reset_insight_seen(*, insight_id: uuid.UUID, username: str | None) -> None:
    """Lets a dismissed insight show again — for one account (--user) or every account that had
    seen it (omit --user). Per-item analog of enable-whats-new/enable-tours, decoupled from the
    content-level enable-insight/disable-insight toggle above (see `Insight`'s own doc comment for
    why those are two different axes for this feature specifically)."""
    async with async_session_factory() as db:
        insight = await insight_repo.get_by_id(db, insight_id)
        if insight is None:
            print(f"No insight with id {insight_id}.")
        ...
        if username is not None:
            ...
            if user is None:
                print(f"No user named {username!r}.")
                return
            await insight_seen_repo.reset_seen_for_user(db, user.id, insight_id)
            print(f"Insight {insight_id} will show again for {username!r}.")
        else:
            await insight_seen_repo.reset_seen_for_all(db, insight_id)
            print(f"Insight {insight_id} will show again for every account that had already seen it.")
...
def _parse_calendar_rows(
    ...
    for line_no, data in _read_csv_rows(file_path):
        ...
        if payload.cancellation_reason and not payload.cancelled:
            ...
            continue
        if payload.cancelled_by and not payload.cancelled:
            errors.append(f"row {line_no}: cancelled_by requires cancelled=true")
            continue
...
async def export_calendar(*, file_path: Path) -> None:
    async with async_session_factory() as db:
        ...
        # Same id set covers both columns — cancelled_by is a real user-id string (or the "both"
        # sentinel, never a real id to resolve) on exactly the same footing as created_by.
        user_ids = {event.created_by for event in events}
        user_ids.update(
            uuid.UUID(event.cancelled_by)
            for event in events
            if event.cancelled_by and event.cancelled_by != CANCELLED_BY_BOTH
        )
        usernames: dict[uuid.UUID, str] = {}
        for user_id in user_ids:
            resolved_user = await user_repo.get_by_id(db, user_id)
            usernames[user_id] = resolved_user.username if resolved_user else ""
    ...
    with file_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for event in events:
            if not event.cancelled_by:
                cancelled_by_value = ""
            elif event.cancelled_by == CANCELLED_BY_BOTH:
                cancelled_by_value = CANCELLED_BY_BOTH
            else:
                cancelled_by_value = usernames.get(uuid.UUID(event.cancelled_by), "")
            writer.writerow(
                [
                    event.id,
                    event.type.value,
...
async def import_calendar(
    *, file_path: Path, username: str, dry_run: bool, delete_missing: bool, skip_confirm: bool
    ...
    async with async_session_factory() as db:
        ...
        # cancelled_by travels through the CSV as a username (matching export_calendar's own
        # "cancelled_by" column, and the same human-readable-identifier convention created_by's
        # export column already uses) or the literal "both" sentinel — resolved here to the real
        # id calendar_service._validate_cancelled_by expects, same shape as CalendarEventCreate
        # itself. Needs the open db session, so it can't happen inside _parse_calendar_rows
        # (deliberately DB-free, called before this session exists) the way the pure
        # cancelled/cancellation_reason checks there do.
        cancelled_by_errors: list[str] = []
        resolved_parsed: list[tuple[int, uuid.UUID | None, CalendarEventCreate]] = []
        for line_no, row_id, payload in parsed:
            raw_cancelled_by = payload.cancelled_by
            if not raw_cancelled_by or raw_cancelled_by == CANCELLED_BY_BOTH:
                resolved_parsed.append((line_no, row_id, payload))
                continue
            cancelled_by_user = await user_repo.get_by_username(db, raw_cancelled_by)
            if cancelled_by_user is None or cancelled_by_user.id not in (user.id, user.partner_id):
                cancelled_by_errors.append(
                    f"row {line_no}: cancelled_by {raw_cancelled_by!r} must be 'both' "
                    "or one of the paired partners' usernames"
                )
                continue
            resolved_parsed.append(
                (line_no, row_id, payload.model_copy(update={"cancelled_by": str(cancelled_by_user.id)}))
            )
        if cancelled_by_errors:
            for error in cancelled_by_errors:
                print(error)
            print(f"{len(cancelled_by_errors)} row(s) failed validation — nothing imported.")
            return
...
def main() -> None:
    ...
    add_insight_parser = subparsers.add_parser(
        "add-insight", help="Create a new DIX AI insight (manually-authored one-liner copy)"
    )
    add_insight_parser.add_argument("--text", required=True, help="The insight's one-liner copy")
    add_insight_parser.add_argument(
        "--user", help="Username to scope this insight to — omit for global (both accounts)"
    )

    list_insights_parser = subparsers.add_parser(
        "list-insights", help="List every DIX AI insight (id, enabled state, target, seen-by count)"
    )
    list_insights_parser.add_argument(
        "--enabled-only", action="store_true", help="Only show enabled insights (default: show all, including disabled)"
    )

    delete_insight_parser = subparsers.add_parser(
        "delete-insight", help="Permanently delete a DIX AI insight (cascades its seen-state for every account)"
    )
    delete_insight_parser.add_argument("--id", type=uuid.UUID, required=True, help="Insight id, from list-insights")

    enable_insight_parser = subparsers.add_parser(
        "enable-insight",
        help="Make a DIX AI insight eligible to be shown again (content-level toggle, not per-user)",
    )
    enable_insight_parser.add_argument("--id", type=uuid.UUID, required=True, help="Insight id, from list-insights")

    disable_insight_parser = subparsers.add_parser(
        "disable-insight",
        help="Stop offering a DIX AI insight to anyone who hasn't seen it yet (content-level toggle, not per-user)",
    )
    disable_insight_parser.add_argument("--id", type=uuid.UUID, required=True, help="Insight id, from list-insights")

    reset_insight_seen_parser = subparsers.add_parser(
        "reset-insight-seen",
        help="Let a dismissed DIX AI insight show again, for one account or everyone who's seen it",
    )
    reset_insight_seen_parser.add_argument("--id", type=uuid.UUID, required=True, help="Insight id, from list-insights")
    reset_insight_seen_parser.add_argument(
        "--user", help="Username to reset just for — omit to reset for every account that had seen it"
    )
    ...
    if args.command == "reset-all":
        ...
        asyncio.run(add_insight(insight_text=args.text, username=args.user))
    elif args.command == "list-insights":
        asyncio.run(list_insights(include_disabled=not args.enabled_only))
    elif args.command == "delete-insight":
        asyncio.run(delete_insight(insight_id=args.id))
    elif args.command == "enable-insight":
        asyncio.run(set_insight_enabled(insight_id=args.id, enabled=True))
    elif args.command == "disable-insight":
        asyncio.run(set_insight_enabled(insight_id=args.id, enabled=False))
    elif args.command == "reset-insight-seen":
        asyncio.run(reset_insight_seen(insight_id=args.id, username=args.user))
