...
# Sentinel for "cancelled_by" meaning "both partners cancelled it together" — the other valid
# values are a real user-id string, validated against the pair's own two ids in calendar_service.
# Own constant rather than reusing intimacy_log.py's identically-valued INITIATED_BY_BOTH — same
# "one sentinel per field/concern" convention that module already established.
CANCELLED_BY_BOTH = "both"
...
class CalendarEvent(UUIDPrimaryKeyMixin, CreatedAtMixin, UpdatedAtMixin, Base):
    ...
    # A real user-id string (one of the pair's own two ids) or CANCELLED_BY_BOTH — same shape as
    # IntimacyLog.initiated_by, validated in calendar_service._validate_cancelled_by.
