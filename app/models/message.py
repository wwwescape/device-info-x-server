...
class Message(UUIDPrimaryKeyMixin, CreatedAtMixin, Base):
    ...
    # LOCATION-only — a one-time GPS snapshot captured at send time, same "unused on every other
    # type" treatment as the poll_*/link_preview_* fields above.
    location_lat: Mapped[float | None] = mapped_column(Float, nullable=True)
    location_lng: Mapped[float | None] = mapped_column(Float, nullable=True)
    # Shared by every sibling message in a multi-select photo/video "album" send (client-generated,
    # one id per batch) — a tag among independent message rows, not a relation to another table, so
    # no FK. Null means "not part of an album," true for every message type except IMAGE/VIDEO.
    media_group_id: Mapped[uuid.UUID | None] = mapped_column(PGUUID(as_uuid=True), nullable=True, index=True)
