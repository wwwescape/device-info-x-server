...
class LockerItem(UUIDPrimaryKeyMixin, CreatedAtMixin, UpdatedAtMixin, Base):
    ...
    # Set only when this item was saved from an existing message/event attachment (never for a
    # direct gallery/document import, which leaves all three null) — lets the server enforce the
    # same "already saved" dedupe the client already checks locally, closing the race a
    # double-tap on Save-to-Vault could otherwise hit (two concurrent saves both seeing "not yet
    # saved" before either commits). Each save re-uploads a brand-new media asset rather than
    # reusing the original attachment's, so media_id alone can't be the dedupe key.
    original_message_id: Mapped[uuid.UUID | None] = mapped_column(PGUUID(as_uuid=True), nullable=True)
    original_event_id: Mapped[uuid.UUID | None] = mapped_column(PGUUID(as_uuid=True), nullable=True)
    original_attachment_media_id: Mapped[uuid.UUID | None] = mapped_column(PGUUID(as_uuid=True), nullable=True)
