...
def _send_test_sync(tokens: list[str]) -> tuple[int, list[str]]:
    """Runs in a worker thread. Unlike the data-only messages the rest of this module sends,
    this includes a `notification` block so FCM auto-displays it — verifies actual delivery to
    the device independent of the Android app's own (data-only-driven) rendering logic. Only
    ever called from the `test-notification` CLI command, never from a real notification path.
    Returns (success_count, invalid_tokens) — invalid_tokens are ones FCM itself reports as
    permanently unregistered (e.g. superseded by a newer token issued to the same install, which
    happens on every rebuild/reinstall during development — a token's *age* says nothing about
    whether it's still valid, only FCM's own response does)."""
    message = messaging.MulticastMessage(
