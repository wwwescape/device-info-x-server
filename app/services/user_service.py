...
async def update_profile(
    ...
    *,
    ...
    if birthday_message is not None:
        # "" is a valid, meaningful value here (clears it — no message shown on the partner's
        # popup), unlike every other field above where None-means-skip is the only distinction
        # that matters. The client always sends the current text explicitly when saving this
        # section, so None only ever means "this field wasn't touched by this request".
        user.birthday_message = birthday_message or None
    await db.commit()
...
async def list_seen_tour_keys(db: AsyncSession, user: User) -> list[str]:
    return await feature_tour_seen_repo.list_tour_keys_for_user(db, user.id)


async def mark_tour_seen(db: AsyncSession, user: User, tour_key: str) -> None:
    await feature_tour_seen_repo.mark_seen(db, user.id, tour_key)


async def list_seen_whats_new_tags(db: AsyncSession, user: User) -> list[str]:
    return await whats_new_seen_repo.list_tags_for_user(db, user.id)


async def mark_whats_new_seen(db: AsyncSession, user: User, tag: str) -> None:
    await whats_new_seen_repo.mark_seen(db, user.id, tag)


async def list_unseen_insights(db: AsyncSession, user: User) -> list[Insight]:
    """Every enabled DIX AI insight visible to this account (global or targeted at just this
    account, see `insight_repo.list_visible_to`) that this account hasn't dismissed yet."""
    visible = await insight_repo.list_visible_to(db, user.id)
    seen_ids = set(await insight_seen_repo.list_seen_ids_for_user(db, user.id))
    return [insight for insight in visible if insight.id not in seen_ids]


async def mark_insight_seen(db: AsyncSession, user: User, insight_id: uuid.UUID) -> None:
    await insight_seen_repo.mark_seen(db, user.id, insight_id)
