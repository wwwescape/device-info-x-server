...
async def issue_token_pair(
    ...
    # Deliberately does NOT touch user.last_seen_at here. This fires on every token refresh,
    # including fully passive, backgrounded ones with no real user activity — e.g.
    # ConsoleFcmService.ackMessageDelivered's silent delivery-ack POST, triggered purely by
    # receiving a push while the app is closed, which transparently refreshes an expired access
    # token via AuthAuthenticator. last_seen_at has exactly one legitimate writer,
    # presence_service.mark_offline (a real WebSocket disconnect) — writing it here too let an
    # unrelated background API call masquerade as "last seen," e.g. showing the partner's last
    # seen as the exact moment *you* sent them a message rather than when they actually last had
    # the app open. Confirmed and root-caused via a live repro, not just code review.
    if timezone:
