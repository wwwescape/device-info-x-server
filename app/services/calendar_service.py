...
def _validate_cancelled_allowed(
    event_type: EventType, cancelled: bool, cancellation_reason: str | None, cancelled_by: str | None
) -> None:
    if cancelled and event_type not in CANCELLABLE_TYPES:
        raise ValidationError(f"{event_type.value} events cannot be cancelled")
    # A reason/cancelled_by implies cancelled=True, which above already narrows the type down to
    # CANCELLABLE_TYPES — no separate type check needed for either field itself.
    if cancellation_reason and not cancelled:
        raise ValidationError("cancellation_reason can only be set on a cancelled event")
    if cancelled_by and not cancelled:
        raise ValidationError("cancelled_by can only be set on a cancelled event")


def _validate_cancelled_by(cancelled_by: str | None, user: User) -> None:
    if cancelled_by is None:
        return
    valid = {CANCELLED_BY_BOTH, str(user.id)}
    if user.partner_id is not None:
        valid.add(str(user.partner_id))
    if cancelled_by not in valid:
        raise ValidationError("cancelled_by must be 'both' or one of the paired partners")
...
async def _validate_cover_media_id(db: AsyncSession, user: User, media_id: uuid.UUID | None) -> None:
    """Singular sibling of [_validate_attachment_ids] — a cover photo must be the user's own
    upload, of the CALENDAR_IMAGE category specifically (not video/document, unlike the general
    attachments list)."""
    if media_id is None:
        return
    asset = await media_asset_repo.get_by_id(db, media_id)
    if asset is None or asset.owner_id != user.id or asset.category != MediaCategory.CALENDAR_IMAGE:
        raise ValidationError("cover_media_id must reference your own calendar_image upload")
...
def _to_out(
    ...
    return CalendarEventOut(
        ...
        cover_media=MediaAssetOut.model_validate(cover_media) if cover_media is not None else None,
...
async def create_event(db: AsyncSession, user: User, payload: CalendarEventCreate) -> CalendarEventOut:
    ...
    _validate_cancelled_allowed(payload.type, payload.cancelled, payload.cancellation_reason, payload.cancelled_by)
    _validate_cancelled_by(payload.cancelled_by, user)
    ...
    await _validate_cover_media_id(db, user, payload.cover_media_id)
    ...
    cover_media = await media_asset_repo.get_by_id(db, payload.cover_media_id) if payload.cover_media_id else None
    out = _to_out(
        event, payload.reminder_minutes_before, _intimacy_out_from_payload(intimacy_data), attachments, cover_media
    )
    if user.partner_id is not None:
...
async def update_event(
    ...
    new_cancelled_by = data.get("cancelled_by", event.cancelled_by)
    if new_rule:
        _validate_rrule(new_rule, new_start)
    _validate_recurrence_allowed(new_type, new_rule)
    _validate_cancelled_allowed(new_type, new_cancelled, new_cancellation_reason, new_cancelled_by)
    _validate_cancelled_by(new_cancelled_by, user)
    ...
    if "cover_media_id" in data:
        await _validate_cover_media_id(db, user, data["cover_media_id"])
    ...
    cover_media = await media_asset_repo.get_by_id(db, event.cover_media_id) if event.cover_media_id else None

    out = _to_out(event, reminder_minutes, intimacy_out, attachments, cover_media)
    if user.partner_id is not None:
        await notification_service.notify_user(
            user.partner_id,
...
async def list_events(db: AsyncSession, *, start: datetime, end: datetime) -> list[CalendarEventOut]:
    ...
    cover_media_map = await calendar_repo.cover_media_for_events(db, event_ids)
    return [
        _to_out(
            e,
...
async def get_event(db: AsyncSession, event_id: uuid.UUID) -> CalendarEventOut:
    ...
    attachments = (await calendar_repo.attachments_for_events(db, [event.id])).get(event.id, [])
    cover_media = await media_asset_repo.get_by_id(db, event.cover_media_id) if event.cover_media_id else None
    return _to_out(event, reminders, _intimacy_out_from_orm(intimacy), attachments, cover_media)
