...
async def delete_all_own_data(db: AsyncSession, current_user: User) -> None:
    ...
    DB writes (item delete, album delete, and the now-orphaned media_assets rows) all land in one
    commit; the on-disk blobs are only removed *after* that commit succeeds, so a mid-sweep
    ...
    for item in items:
        if item.media_id is None:
            continue
        asset = await media_asset_repo.get_by_id(db, item.media_id)
