...
async def create_item(db: AsyncSession, current_user: User, payload: LockerItemCreate) -> LockerItemOut:
    ...
    try:
        item = await locker_repo.create(
            ...
            original_attachment_media_id=payload.original_attachment_media_id,
        )
        await db.commit()
    except IntegrityError:
        # A concurrent duplicate save (e.g. a double-tap the client-side dedupe guard didn't
        # catch) raced this insert against locker_items' unique dedupe indexes — treat it the
        # same as message_service.send_message's own "already exists, return it" idempotent
        # handling, just reacting to the DB conflict instead of a pre-check. Not a new item, so
        # no partner notification below either.
        await db.rollback()
        existing = None
        if payload.original_message_id is not None:
            existing = await locker_repo.get_by_original_message(db, current_user.id, payload.original_message_id)
        elif payload.original_event_id is not None and payload.original_attachment_media_id is not None:
            existing = await locker_repo.get_by_original_event_attachment(
                db, current_user.id, payload.original_event_id, payload.original_attachment_media_id
            )
        if existing is None:
            raise
