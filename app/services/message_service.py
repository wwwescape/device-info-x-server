...
async def send_message(db: AsyncSession, sender: User, payload) -> MessageOut:
    ...
    if payload.type == MessageType.LOCATION:
        if payload.location_lat is None or payload.location_lng is None:
            raise ValidationError("location messages require location_lat and location_lng")
        if not (-90 <= payload.location_lat <= 90) or not (-180 <= payload.location_lng <= 180):
            raise ValidationError("location_lat/location_lng out of range")
