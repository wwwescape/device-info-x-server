...
async def wipe_conversation(db: AsyncSession, current_user: User) -> None:
    ...
    DB writes (the message hard-delete + the now-orphaned media_assets rows) land in one commit;
    on-disk blobs are only removed after that commit succeeds — same ordering as
