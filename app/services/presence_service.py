...
from app.services import call_room_service, game_room_service, notification_service
...
async def mark_offline(user_id: uuid.UUID) -> None:
    ...
    if partner_id is not None:
        ...
        # Same "disconnect never sends an explicit stop first" gap as the "Here" tier just above —
        # ConsoleWebSocketClient.reportSelfTyping only ever fires from ComposerBar's own text-change
        # callback, so a killed/backgrounded app with a non-blank draft still in the composer never
        # gets to send typing.stop. Without this, the partner's already-connected device is left
        # showing a stale typing indicator for the rest of that session — same failure shape, so the
        # same unconditional-push-on-disconnect fix, regardless of whether this user was actually
        # mid-typing (a stray stop when they weren't is a no-op on the receiving end either way).
        await notification_service.notify_user(
            partner_id,
            "typing",
            {"user_id": str(user_id), "state": "stop"},
        )
    ...
    # Same treatment for the Call Room — also how "ringing/an active call ends the instant either
    # partner exits the app" (CALLING_PLAN.md §13) is actually enforced for a killed/backgrounded
    # app, not just an explicit screen.call_room_leave.
    await call_room_service.handle_disconnect(user_id)
