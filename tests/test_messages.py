...
async def test_media_group_id_round_trips_across_sibling_messages(client, paired):
    alice, bob = paired
    ...
    for filename in ("one.jpg", "two.jpg"):
        ...
        assert r.status_code == 201, r.text
        assert r.json()["media_group_id"] == group_id
        sent_ids.append(r.json()["id"])

    r = await client.get("/api/v1/messages", headers=auth_headers(bob))
    assert r.status_code == 200
    by_id = {m["id"]: m for m in r.json()}
    assert by_id[sent_ids[0]]["media_group_id"] == group_id
    assert by_id[sent_ids[1]]["media_group_id"] == group_id

    # a message sent without one still comes back null, not an accidental default
    r = await client.post(
        "/api/v1/messages",
        json={"type": "text", "body": "not in an album", "client_message_id": str(uuid.uuid4())},
        headers=auth_headers(alice),
    )
    assert r.status_code == 201
    assert r.json()["media_group_id"] is None
