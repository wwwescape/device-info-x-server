...
async def test_cancelled_by_both_round_trips(client, paired):
    alice, _bob = paired
    r = await client.post(
        "/api/v1/calendar/events",
        json={
    ...
    assert r.status_code == 201, r.text
    assert r.json()["cancelled_by"] == "both"


async def test_cancelled_by_partner_id_round_trips(client, paired):
    alice, bob = paired
    r = await client.post(
        "/api/v1/calendar/events",
        json={
    ...
    assert r.status_code == 201, r.text
    assert r.json()["cancelled_by"] == bob["id"]


async def test_cancelled_by_rejected_without_cancelled(client, paired):
    alice, _bob = paired
    r = await client.post(
        "/api/v1/calendar/events",
        json={
...
async def test_cancelled_by_rejected_for_non_paired_user(client, paired):
    alice, _bob = paired
    r = await client.post(
        "/api/v1/calendar/events",
        json={
...
async def test_cancelled_by_updatable_and_cleared_when_uncancelled(client, paired):
    alice, bob = paired
    r = await client.post(
        "/api/v1/calendar/events",
        json={
            ...
            "start_at": "2026-09-01T18:00:00Z",
            "cancelled": True,
            "cancelled_by": alice["id"],
        },
        headers=auth_headers(alice),
    )
    event_id = r.json()["id"]
    assert r.json()["cancelled_by"] == alice["id"]

    r = await client.patch(
        f"/api/v1/calendar/events/{event_id}",
        json={"cancelled_by": "both"},
        headers=auth_headers(bob),
    )
    assert r.status_code == 200
    assert r.json()["cancelled_by"] == "both"
    ...
    r = await client.patch(
        f"/api/v1/calendar/events/{event_id}",
        json={"cancelled": False, "cancelled_by": None},
        headers=auth_headers(bob),
    )
    assert r.status_code == 200
    assert r.json()["cancelled"] is False
    assert r.json()["cancelled_by"] is None
