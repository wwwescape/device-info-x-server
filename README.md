...
10. [HTTPS, Let's Encrypt, and the reverse proxy](#https-lets-encrypt-and-the-reverse-proxy)
11. [Port forwarding and Dynamic DNS](#port-forwarding-and-dynamic-dns)
12. [Coturn (TURN/STUN)](#coturn-turnstun)
13. [Pairing your two accounts](#pairing-your-two-accounts)
14. [Resetting all data](#resetting-all-data)
15. [Resetting a forgotten password](#resetting-a-forgotten-password)
16. [Registering an account for your partner](#registering-an-account-for-your-partner)
17. [Testing push notifications](#testing-push-notifications)
18. [CSV data import](#csv-data-import)
19. [Backups and restore](#backups-and-restore)
20. [Updating containers](#updating-containers)
21. [Logs](#logs)
22. [Troubleshooting](#troubleshooting)
23. [Security hardening](#security-hardening)
24. [Monitoring](#monitoring)
25. [Production recommendations](#production-recommendations)
...
| `MAX_IMAGE_SIZE_MB` / `MAX_VOICE_SIZE_MB` / `MAX_AVATAR_SIZE_MB` / `MAX_LOCKER_FILE_SIZE_MB` | Per-category upload caps. If you raise these, also raise `client_max_body_size` in `nginx/nginx.conf`. |
...
For backfilling historical calendar events or period cycles in bulk (e.g.
...
[`samples/period_cycles_example.csv`](samples/period_cycles_example.csv) and
...
cp samples/period_cycles_example.csv imports/period_cycles.csv
```
...
docker exec -it deviceinfoxserver_api python -m app.cli export-period \
  --file /data/imports/period_cycles.csv --user alice
...
| `cycle_start_date` | Yes | `YYYY-MM-DD` | |
| `cycle_length_days` | No | integer, 1–90 | |
| `period_end_date` | No | `YYYY-MM-DD` | |
...
period cycles it's whose data every row belongs to. For calendar events
...
- An **existing record whose `id` isn't anywhere in the file** is left alone
  ...
  compares against **everything** currently in scope — every cycle that
...
docker exec -it deviceinfoxserver_api python -m app.cli import-period \
  --file /data/imports/period_cycles.csv --user alice --dry-run

# Add/update rows, and remove anything no longer in the file:
docker exec -it deviceinfoxserver_api python -m app.cli import-period \
  --file /data/imports/period_cycles.csv --user alice --delete-missing
```
